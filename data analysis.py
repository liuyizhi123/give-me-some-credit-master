# -*- coding: utf-8 -*-
"""
Created on Thu Mar   10:49:36 2019
...
pd.set_option("display.max_columns",101)#打印出来最多显示101列
#lambda生成匿名函数。%.5f：表示按浮点数输出，小数点后面取5位其余的舍去。
pd.set_option('display.float_format', lambda x: '%.5f' % x) #为了直观的显示数字，不采用科学计数法
pd.options.display.max_rows = 15 #打印出来最多显示15行
import warnings
warnings.filterwarnings('ignore') #为了整洁，去除弹出的warnings

df = pd.read_csv('data/cs-training.csv')#读取cs-training.csv文件·
#print(df.describe())
